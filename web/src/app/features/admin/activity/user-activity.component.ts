...
export class UserActivityComponent implements OnInit {
  ...
  loadActivity(): void {
    ...
    const [sort, order] =
      this.sortBy === 'oldest' ? ['createdAt', 'asc' as const] : ['createdAt', 'desc' as const];
  ...
  getActionIcon(action: string): string {
    const icons: Record<string, string> = {
      ...
      page_view: 'web',
      favorite: 'favorite',
      unfavorite: 'heart_broken',
      contact: 'call',
      share: 'share',
      listing_create: 'add_circle',
      listing_edit: 'edit',
      listing_delete: 'delete',
      listing_status_change: 'swap_horiz',
      listing_feature: 'star',
      login: 'login',
      register: 'person_add',
      logout: 'logout',
      message_sent: 'send',
      conversation_start: 'chat',
      package_purchase: 'shopping_cart',
      payment_attempt: 'payment',
      location_change: 'location_on',
      dismiss: 'close',
      recommendation_click: 'recommend',
      admin_user_status_change: 'manage_accounts',
      admin_user_role_change: 'admin_panel_settings',
      admin_user_listing_limit_change: 'tune',
      admin_listing_approve: 'check_circle',
      admin_listing_reject: 'cancel',
      admin_category_create: 'create_new_folder',
      admin_category_update: 'edit_note',
      admin_category_delete: 'folder_delete',
      admin_category_attributes_update: 'list_alt',
      admin_category_features_update: 'checklist',
      admin_location_create: 'add_location',
      admin_location_update: 'edit_location_alt',
      admin_location_delete: 'wrong_location',
      admin_package_create: 'add_shopping_cart',
      admin_package_update: 'inventory',
      admin_export_report: 'download',
    };
  ...
  getActionColor(action: string): string {
    const colors: Record<string, string> = {
      ...
      register: '#00B894',
      logout: '#636e72',
      listing_create: '#0DA5C3',
      listing_edit: '#FDCB6E',
      listing_delete: '#E74C3C',
      listing_status_change: '#6C5CE7',
      listing_feature: '#F39C12',
      favorite: '#E74C3C',
      unfavorite: '#636e72',
      message_sent: '#0DA5C3',
      conversation_start: '#0DA5C3',
      package_purchase: '#00B894',
      payment_attempt: '#F39C12',
      search: '#6C5CE7',
      contact: '#00B894',
      admin_user_status_change: '#E74C3C',
      admin_user_role_change: '#6C5CE7',
      admin_user_listing_limit_change: '#F39C12',
      admin_listing_approve: '#00B894',
      admin_listing_reject: '#E74C3C',
      admin_category_create: '#0DA5C3',
      admin_category_update: '#FDCB6E',
      admin_category_delete: '#E74C3C',
      admin_category_attributes_update: '#6C5CE7',
      admin_category_features_update: '#6C5CE7',
      admin_location_create: '#0DA5C3',
      admin_location_update: '#FDCB6E',
      admin_location_delete: '#E74C3C',
      admin_package_create: '#00B894',
      admin_package_update: '#F39C12',
      admin_export_report: '#636e72',
    };
  ...
  getSummary(entry: ActivityEntry): string {
    ...
    switch (entry.action) {
      case 'login':
        return `via ${m['method'] || 'unknown'} • ${m['browser'] || ''} on ${m['os'] || ''}`.trim();
      case 'logout':
        return `${m['browser'] || ''} on ${m['os'] || ''}`.trim() || 'Session ended';
      case 'search':
        return entry.searchQuery ? `"${entry.searchQuery}"` : '';
      case 'listing_status_change':
        return m['previousStatus'] ? `${m['previousStatus']} → ${m['newStatus']}` : '';
      case 'location_change':
        return m['newLocation'] || m['locationCity'] || '';
      case 'contact':
        return m['type'] === 'call' ? 'Phone call' : m['type'] === 'message' ? 'Message' : '';
      case 'listing_create':
      case 'listing_edit':
        return m['title'] || '';
  ...
  getDetailSections(
    entry: ActivityEntry,
  ): { label: string; icon: string; items: { key: string; value: any }[] }[] {
    const m = entry.metadata || {};
    ...
    if (entry.productListingId)
      contextItems.push({ key: 'Listing ID', value: entry.productListingId });
    ...
    if (deviceItems.length)
      sections.push({ label: 'Device & Browser', icon: 'devices', items: deviceItems });
