...
describe('AdminService', () => {
  ...
  let httpMock: { get: ReturnType<typeof vi.fn>; patch: ReturnType<typeof vi.fn>; post: ReturnType<typeof vi.fn> };
  ...
  const mockAnalytics: AnalyticsData = {
    ...
    timeSeries: {
      registrations: [{ date: '2024-01-01', value: 10 }, { date: '2024-01-02', value: 15 }],
  ...
  it('should call GET /admin/users with params', () => {
    ...
    service.getUsers({ page: 1, limit: 10, search: 'alice', role: 'seller', status: 'active' }).subscribe((data) => {
  ...
  it('should call GET /admin/listings/pending', () => {
    ...
    expect(httpMock.get).toHaveBeenCalledWith(
      expect.stringContaining('/admin/listings/pending'),
    );
  });
  ...
  it('should call PATCH /packages/:id to update a package', () => {
    ...
    expect(httpMock.patch).toHaveBeenCalledWith(
      expect.stringContaining('/packages/p1'),
      { name: 'Updated' },
    );
  });
  ...
  it('should call GET /admin/packages/purchases with params', () => {
    ...
    httpMock.get.mockReturnValue(of(mockRes));
    service.getAdminPurchases({ page: 1, limit: 10, type: 'featured_ads', status: 'completed' }).subscribe((data) => {
      expect(data).toEqual(mockRes);
    });
    expect(httpMock.get).toHaveBeenCalledWith(
  ...
  it('should call GET /admin/payments with params', () => {
    ...
    httpMock.get.mockReturnValue(of(mockRes));
    service.getAdminPayments({ page: 1, limit: 15, paymentMethod: 'card', status: 'completed' }).subscribe((data) => {
      expect(data).toEqual(mockRes);
    });
    expect(httpMock.get).toHaveBeenCalledWith(
